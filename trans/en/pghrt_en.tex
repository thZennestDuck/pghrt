...
One thing to note: if you are swapping needles for drawing and injecting, then you should pull the plunger back slightly prior to taking off the drawing needle so that the fluid inside the drawing needle is not wasted. It is very minor, but it can sometimes make a difference. \textbf{Note as well that syringe markings measure volume in the syringe *and* the volume of dead space, so you should not manually add more to your injection to adjust for dead space.} The exact volume of dead space depends on the particular needle (i.e., a drawing and injecting needle are unlikely to have equal dead space volumes), so the important thing as always is consistency. See Question \ref{7-7} for another possible strategy if concerned about high dead space.
